...
tp_layer = isotp.TransportLayer(address=addr1, error_handler=my_error_handler, params=params1, rxfn=my_rxfn_1, txfn=my_txfn_1)
...
can_stack = isotp.CanStack(bus2, address=addr2, error_handler=my_error_handler, params=params2)
...
def main():
    ...
    t_bus1_task = threading.Thread(target=bus1_task, args=(tp_layer, stop_event))
    t_bus1_task.start()
    
    t_bus2_task = threading.Thread(target=bus2_task, args=(can_stack, stop_event))
    t_bus2_task.start()
